...
class LcdI2c:
  ...
  def allowed(self):
    self.__print_on_lcd(["Belepes", "engedelyezve!"])

  def denied_generic(self):
    self.__print_on_lcd(["Belepes", "megtagadva!"])

  def denied_locked(self):
    self.__print_on_lcd(["Kartya", "letiltva!"])

  def denied_unknown(self):
    self.__print_on_lcd(["Kartya nem", "ismert!"])

  def __print_on_lcd(self, messages: list[str]):
